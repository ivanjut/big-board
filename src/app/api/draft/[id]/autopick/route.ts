...
export async function POST(
  ...
  try {
    await db`
      insert into picks
        (draft_id, pick_number, round, slot, player_id, player_name, player_position, player_team, was_skipped, auto_picked)
      values
        (${id}, ${targetPick}, ${round}, ${slot}, ${player.id}, ${player.name}, ${player.position}, ${player.team}, ${skippedSet.has(targetPick)}, true)
    `;
  } catch (e) {
